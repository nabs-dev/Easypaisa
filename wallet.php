...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (password_verify($pin, $user['pin'])) {
        ...
        $stmt->execute([$type == 'deposit' ? $amount : -$amount, $_SESSION['user_id']]);
        
        $stmt = $conn->prepare("INSERT INTO transactions (user_id, type, amount, status) VALUES (?, ?, ?, 'completed')");
        $stmt->execute([$_SESSION['user_id'], $type, $amount]);
        
        echo "<script>alert('$type successful!'); window.location.href='wallet.php';</script>";
    } else {
